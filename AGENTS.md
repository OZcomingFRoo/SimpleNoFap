...
- Room/SQLite for structured local records such as scheduled notifications and Day-Streak rewards.
...
- Room is the local structured database for scheduled notifications and Day-Streak rewards.
- Repositories are the boundary between storage and the rest of the app. UI and domain code must not access Room DAOs directly.
- DAOs should stay persistence-only. Validation, business rules, and app-owned mappings belong in repositories or domain helpers.
...
- Room database entrypoint: `SimpleNoFapDatabase` in `com.example.simplenofap.data.local`.
- Room entities include autogenerated `id: Long`, `createdAtEpochMillis: Long`, and `updatedAtEpochMillis: Long`.
- `scheduled_notifications` stores notification schedules: `daysOfWeekMask`, `timeMinutesOfDay`, preset/custom message fields, preset/custom title fields, and `active`.
- `daysOfWeekMask` uses Sunday as bit `1`, Monday `2`, Tuesday `4`, Wednesday `8`, Thursday `16`, Friday `32`, and Saturday `64`. Valid masks are `1..127`.
- `timeMinutesOfDay` is an integer minute of day. Valid values are `0..1439`.
- `day_streak_rewards` stores earned Day-Streak rewards: `streakType`, `achievedAtEpochMillis`, nullable `usedAtEpochMillis`, and nullable `sourceStreakStartAtEpochMillis`.
- `usedAtEpochMillis == null` means a Day-Streak reward is unused/available.
- Do not persist localized names, localized notification preset text, or icons in Room. Resolve notification presets and Day-Streak names/icons from app-owned mappings and localization.
- Current repositories are `NotificationRepository` and `DayStreakRepository`. Add future structured data access behind repositories before wiring UI or scheduling workflows.
